...
function AjaxCreateOrder(e){
    ...
    request.fail(function(jqXHR, textStatus){
        var msg = 'checkout.js ---(1) : AjaxCreateOrder'
        if (jqXHR.status==404){
...
function AjaxStoreTransaction(e, order_id, amount, type){
    ...
    request.fail(function (jqXHR, textStatus){
        var msg = 'checkout.js ---(2) : AjaxStoreTransaction'
        if (jqXHR.status==404){
            alert("페이지가 존재하지 않습니다.");
        } else if (jqXHR.status == 403){
            console.log(msg);
...
function ImpTransaction(e, order_id, merchant_id, imp_id, amount){
    ...
    request.done(function (data){
        if (data.works) {
            $(loacation).attr('href', loaction.origin + order_complete_url + '?order_id=' + order_id)
        }
    });

    request.fail(function(jqXHR, textStatus){
        var msg = 'checkout.js ---(3) : ImpTransaction'
